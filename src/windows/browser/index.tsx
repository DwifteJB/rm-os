...
const BrowserComponent = () => {
    ...
    const handleSearch = async (event: React.FormEvent) => {
        ...
        if (iframeWindowRef.current) {
            iframeWindowRef.current.src = __uv$config.prefix + __uv$config.encodeUrl(inputUrl);
        }
    };

    useEffect(() => {
        if (iframeWindowRef.current) {
            iframeWindowRef.current.src = __uv$config.prefix + __uv$config.encodeUrl("https://google.com");
        }
    }, [iframeWindowRef])
