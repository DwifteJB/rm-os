...
export const getLanguageNameCloseToExtension = (
  extension: string,
): string | undefined => {
  // useful for creating new file, and can allow user to autopick language
  return Languages.find((lang) =>
    lang.extensions.some((ext) => ext.includes(extension)),
  )?.name;
};

export const getLanguageNameCloseToString = (
  string: string,
):
  | {
      name: string;
...
export const getLanguageNameFromExtension = (
  extension: string,
): string | undefined => {
  // useful for creating new file and allowing monaco to actually work! auto detects language name from extension
  return Languages.find((lang) => lang.extensions.includes(extension))?.name;
};

export const getLanguageExtensionsFromName = (
  name: string,
): string[] | undefined => {
  // useful for getting extensions for monaco editor
